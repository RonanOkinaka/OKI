...
public:
    ...
    DebugHandleGenerator(DebugHandleGenerator<HandleType>&&) noexcept = default;
...
public:
    ...
    ReuseHandleGenerator(const ReuseHandleGenerator<HandleType>&) = delete;
     ...
     * reused later).
     *
     * Returns false if there was an exception thrown when attempting to
     * destroy.
     */
     ...
     *     - It is LIKELY not currently deleted (i.e. it refers to
     * something) However, there are some caveats:
